...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
			...
			if (Game.TickerN%2==0 || loreProgress>14)
			{
				...
				else
				{
					if (Math.random()<0.05)
					{
						...
						if (Game.Has('Kitten assassins')) list.push('News : several local bakers found dead! Police are unsure on who could be behind this.');
		...
		new Game.Object('Sky citadel','sky citadel|sky citadels|coagulated|The sky is [X]% cloudier|The sky is [X]% cloudier','Harvests the air to be extracted and turned into cookies.',19,36,{base:'skycitadel',xV:16,yV:16,w:48,rows:2,x:0,y:20},10000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		Game.NewUnshackleBuilding=function(obj)
		{
			...
			var upgrade=new Game.Upgrade('Unshackled '+building.bplural,(obj.building=='Cursor'?getStrThousandFingersGain(25):loc("Tiered upgrades for <b>%1</b> provide an extra <b>+%2%</b> production.<br>Only works with unshackled upgrade tiers.",[cap(building.plural),Math.round((building.id==1?0.5:(24-building.id)*0.1)*100)]))+(EN?'<q>'+obj.q+'</q>':''),Math.pow(building.id+1,7)*15000000,[building.iconColumn,35]);
		...
		Game.NewUpgradeCookie({name:'Web cookies',desc:'The original recipe; named for the delicate pattern inscribed on their surface by the baking process. Eating these can tell a lot about someone. Invented by well-connected bakers, no doubt.',icon:[25,35],power:						5,price: getCookiePrice(52)});
		Game.NewUpgradeCookie({name:'Steamed cookies',desc:'Localized entirely within this gaming platform? Yes! Baked with the power of steam, in a touch of cutting-edge modernity not seen since the industrial revolution.',icon:[26,35],power:						5,price: getCookiePrice(52)});
		...
		order=100;new Game.Upgrade('Duodecillion fingers',getStrThousandFingersGain(20)+'<q>And then, there was nay but those appendages, and while I stared, I could almost swear I could hear them speak to me...</q>',10000000000000000000000000000000000,[12,37]);Game.MakeTiered(Game.last,16,0);
		...
		order=550;Game.TieredUpgrade('Self-idolatry','<q>After all, there\'s no greater force than your own cookie empire.</q>','Temple',16);
		...
		order=1070;new Game.Achievement('Abhorrent abomination','',[0,27]);Game.Objects['Cursor'].levelAchiev15=Game.last;
		order=1120;new Game.Achievement('At the edge of death','',[1,27]);Game.Objects['Grandma'].levelAchiev15=Game.last;
		order=1220;new Game.Achievement('Biggering','',[2,27]);Game.Objects['Farm'].levelAchiev15=Game.last;
		order=1320;new Game.Achievement('Desecrating the mantle','',[3,27]);Game.Objects['Mine'].levelAchiev15=Game.last;
		order=1420;new Game.Achievement('Copyright abuse','',[4,27]);Game.Objects['Factory'].levelAchiev15=Game.last;
		order=1445;new Game.Achievement('Economical boom','',[15,27]);Game.Objects['Bank'].levelAchiev15=Game.last;
		order=1470;new Game.Achievement('It\’s not the years, honey, it\’s the mileage','',[16,27]);Game.Objects['Temple'].levelAchiev15=Game.last;
		order=1495;new Game.Achievement('Lapsus linguae','',[17,27]);Game.Objects['Wizard tower'].levelAchiev15=Game.last;
		order=1498;new Game.Achievement('When you stare into the sun, it stares back','',[36,27]);Game.Objects['Sky citadel'].levelAchiev15=Game.last;
		order=1520;new Game.Achievement('If it\'s not on a map, it doesn\'t exist','',[5,27]);Game.Objects['Shipment'].levelAchiev15=Game.last;
		order=1620;new Game.Achievement('Quicksilver','',[6,27]);Game.Objects['Alchemy lab'].levelAchiev15=Game.last;
		order=1720;new Game.Achievement('Mr. Mandela','',[7,27]);Game.Objects['Portal'].levelAchiev15=Game.last;
		...
		order=1000;new Game.Achievement('Alas, no room will ever be made for a rat',loc("Make <b>%1</b> from clicking.",loc("%1 cookie",LBeautify(1e29))),[11,36]);
		...
		order=1300;Game.TieredAchievement('Nothing left to discover','','Mine',16);
		order=1400;Game.TieredAchievement('Your very own automated empire','','Factory',16);
		order=1425;Game.TieredAchievement('And the rich get even richer','','Bank',16);
		order=1450;Game.TieredAchievement('ᚪᛚᛚ ᚣᚩᚢ\'ᛚᛚ ᛖᚠᛖᚱ ᚾᛖᛖᛞ','','Temple',16);
		order=1475;Game.TieredAchievement('Can you spell it?','','Wizard tower',16);
		order=1497;Game.TieredAchievement('Tall enough for me','','Sky citadel',16);
		order=1500;Game.TieredAchievement('If you can\'t beat \'em, ship \'em','q>Or, alternatively, eat them.</q>','Shipment',16);
		order=1600;Game.TieredAchievement('Sometimes, you DON\'T want to know how it\'s made','<q>Especially hot dogs.</q>','Alchemy lab',16);
		order=1700;Game.TieredAchievement('Into the flourpus','','Portal',16);
		order=1800;Game.TieredAchievement('The then is now now','<q>But we\'re not sure about the later.</q>','Time machine',16);
		order=1900;Game.TieredAchievement('Does it really matter?','','Antimatter condenser',16);
		order=2000;Game.TieredAchievement('Mantis shrimp vision','<q>.</q>','Prism',16);
		order=2100;Game.TieredAchievement('I like those odds','','Chancemaker',16);
		order=2200;Game.TieredAchievement('The definition of insanity is doing the same thing over and over again','<q>And YOU, my good sir, under this very definition, must be insane...</q>','Fractal engine',16);
